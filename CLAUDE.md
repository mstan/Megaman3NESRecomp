...
**nesrecomp** (the recompiler + runner in the `nesrecomp/` submodule) is an **incomplete tool under active development**. Mega Man 3 is a complex game with 6502 patterns and hardware behaviors not yet fully supported by the recompiler or runner. The generated code may compile and run but produce incorrect logic. Finding and fixing these gaps IS the primary work of this project.
...
- **Game repos** (like this one) contain `game.toml`, `extras.c`, and generated output. `game.toml` provides game-specific hints (dispatch patterns, excluded data regions, extra functions) that the recompiler needs to handle the ROM correctly.
...
1. **Native build TCP server** (port 4372) — recompiled game
2. **Emulator (oracle) TCP server** (port 4373) — Nestopia-based emulated build
...
- Mapper registers (MMC3 bank registers, IRQ latch/counter/enable)
...
# Mega Man 3 Specifics

## Mapper
MMC3 (TxROM) — mapper 4. Bank switching via $8000/$8001. Scanline counter IRQ.

## TCP Ports
- Native: **4372**
- Emulated: **4373**

## Game-Specific TCP Commands
- `mm3_state` — game_mode, sub_mode, stage, weapon, lives, prg banks, frame count
- `sched_state` — coroutine scheduler channel table (4 channels)
- `sched_stack` — 6502 stack dump (32 bytes from SP)

## Coroutine Scheduler
MM3 uses a fiber-based coroutine scheduler (Windows Fibers) for its 4-channel cooperative multitasking engine. The scheduler state lives at RAM $80-$8F (4 bytes per channel). This is implemented in extras.c.

## Build Commands

```batch
# Build recompiler (after code_generator.c changes)
cmake --build F:/Projects/nesrecomp-release/megaman3/nesrecomp/build/recompiler --config Release

# Regenerate game code (run from game project directory)
nesrecomp/build/recompiler/Release/NESRecomp.exe "Mega-Man 3 # NES.NES"

# Build game project (from game project directory)
...
| File | Purpose | Edit? |
|------|---------|-------|
| `extras.c` | Game hooks + coroutine scheduler + TCP commands | Yes |
| `verify_mode.c/h` | Dual-execution verification | Yes |
| `game.toml` | Recompiler config (dispatch, extra functions) | Yes |
| `CMakeLists.txt` | Build configuration | Yes if needed |
| `nestopia_bridge.cpp/h` | C++ bridge to Nestopia oracle | Yes if needed |
| `generated/mega-man-3_full.c` | Recompiled game code (BUILD ARTIFACT) | NEVER |
| `generated/mega-man-3_dispatch.c` | Dispatch table (BUILD ARTIFACT) | NEVER |

## Input Scripts and Save States

### Running with a script
```batch
MegaMan3Recomp.exe "Mega-Man 3 # NES.NES" --script C:/temp/session.txt > C:/temp/stdout.txt 2>&1
```
