...
Usage:
    python tools/nes_compare.py [--native-port 4372] [--emu-port 4373]
                                [--ram-diff] [--context N]
...
class NESDebugClient:
    def __init__(self, host: str = "127.0.0.1", port: int = 4372, label: str = ""):
        self.host = host
        self.port = port
...
GAME_DATA_FIELDS = [
    ...
    ("OAM[0].tile", 10),
    ("OAM[0].attr", 11),
    ("$FF ppuctrl_shadow", 12),
    ("$FE ppumask_shadow", 13),
    ("$AE lives", 14),
    ("$F6 bank_flag", 15),
]
...
# ---------------------------------------------------------------------------
# Frame alignment — uses game_mode ($31) and sub_mode ($46) transitions
# ---------------------------------------------------------------------------

def extract_signature(rec: FrameRecord) -> tuple:
    """Extract a game-state signature for alignment matching.
    Uses game_data fields: $14 (state1), $31 (game_mode), $46 (sub_mode)."""
    return (rec.gd_byte(0), rec.gd_byte(2), rec.gd_byte(3))
...
def main():
    ...
    if not args.no_align:
        ...
        for f, old, new in n_trans[:15]:
            print(f"    frame {f:>5}: ($14,$31,$46) {old} -> {new}")
        if len(n_trans) > 15:
